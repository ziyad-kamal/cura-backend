...
const seedAll = async () => {
    try {
        ...
        const postIds = createdPosts.map((post) => post._id).filter((id) => id !== undefined);
        ...
        const adminIds = createdAdmins.map((a) => a._id).filter((id) => id !== undefined);
        ...
        const vendorIds = vendors.map((v) => v._id).filter((id) => id);
