...
export const indexPostsRepo = async (authId, cursor) => {
    ...
    const connections = await Connection.find({
        ...
        .lean();
    const userIds = connections.map((connection) => connection.sender.toString() === authId ? connection.receiver : connection.sender);
    ...
    const [result] = await Post.aggregate([
        {
            $facet: {
                connectionPosts: [
                    {
                        $match: Object.assign(Object.assign({}, cursorQuery), { user: {
                                $in: userIds,
                            } }),
                    },
                    {
                ...
                publicPosts: [
                    {
                        $match: Object.assign(Object.assign({}, cursorQuery), { visibility: "public", user: {
                                $nin: userIds,
                            } }),
                    },
                    {
    ...
    let feed = [...posts, ...repostDocs].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    const hasMore = result.connectionPosts.length > connectionLimit ||
...
export const storePostRepo = async ({ content, files, user, tags, visibility, }) => {
    const post = await (await Post.create({ user, content, files, tags, visibility })).populate("user", "name.first name.last image userInfo.job");
    return post.toJSON();
};
export const repostPostRepo = async ({ content, post }, authId) => {
...
export const updatePostRepo = async ({ content, files, visibility, tags, _id, }) => {
    await findRecord(Post, { _id });
    return (await Post.findByIdAndUpdate(_id, { content, files, visibility, tags }, {
...
export const likePostRepo = async (_id, authId) => {
...
export const deletePostRepo = async (_id) => {
    ...
    if ((_a = post.files) === null || _a === void 0 ? void 0 : _a.length) {
        await Promise.all(post.files.map((file) => s3Client
