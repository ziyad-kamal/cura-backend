...
export const getCartService = async (req) => {
    ...
    const cart = await Cart.findOne({ userId })
        ...
        .lean();
    if (cart) {
        cart.items = await Promise.all(cart.items.map(async (item) => (Object.assign(Object.assign({}, item), { productId: Object.assign(Object.assign({}, item.productId), { images: await resolveFiles(item.productId.images || [], "public") }) }))));
    }
    return cart !== null && cart !== void 0 ? cart : { items: [], totalPrice: 0, _id: null };
};
export const addToCartService = async (req) => {
    var _a;
    const { productId, quantity, userId } = req.body; // Remove price from destructuring
    const finalUserId = userId || ((_a = req.user) === null || _a === void 0 ? void 0 : _a._id);
    if (!finalUserId) {
    ...
    if (!cart) {
        ...
        const itemIndex = cart.items.findIndex((item) => { var _a; return ((_a = item.productId) === null || _a === void 0 ? void 0 : _a.toString()) === productId; });
        if (itemIndex > -1) {
...
export const clearCartService = async (req) => {
    var _a;
    return await Cart.findOneAndUpdate({
        userId: (_a = req.user) === null || _a === void 0 ? void 0 : _a._id,
    }, {
...
export const syncCartService = async (req) => {
    var _a, _b;
    const userId = (_a = req.user) === null || _a === void 0 ? void 0 : _a._id;
    if (!userId) {
        throw new Error("User ID required");
    ...
    for (const item of items) {
        const prodIdStr = (_b = item.productId) === null || _b === void 0 ? void 0 : _b.toString();
        if (!prodIdStr)
            continue;
