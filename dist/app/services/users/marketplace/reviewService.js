...
export const indexReviewsService = async (req) => {
...
export const checkUserRatingService = async (req) => {
    var _a, _b;
...
export const createReviewService = async (req) => {
    ...
    if (rating != null) {
        ...
        if (existingRating) {
            const error = new Error("You have already rated this product. You can only add comments now.");
            error.statusCode = 400;
            throw error;
        }
    }
    // Create the review/comment
    const review = await Review.create(Object.assign(Object.assign({ userId,
        productId }, (rating != null ? { rating } : {})), (comment ? { comment } : {})));
...
export const updateReviewService = async (req) => {
    ...
    if (review.userId.toString() !== (userId === null || userId === void 0 ? void 0 : userId.toString())) {
        const error = new Error("Unauthorized: You do not own this review");
    ...
    return await Review.findById(updatedReview === null || updatedReview === void 0 ? void 0 : updatedReview._id).populate("userId", "name image").populate("productId");
};
